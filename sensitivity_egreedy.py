...
# UCB1
outdir = './outputs/sensitivity/egreedy.txt'
...
for cc in range(1, C_MAX, 1):
    ...
    for i in range(ITERATION):
        command = "python ./gym_adserver/agents/epsilon_greedy_agent.py --num_ads " + str(ADNUM) + " --impressions " + str(EPOCHS) + " --epsilon " + str(ccc)
        print(command)
        os.system(command)
...
lns2 = ax1.plot(x, anss, color='purple', label='Epsilon Greedy')
